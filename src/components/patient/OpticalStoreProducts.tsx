...
const OpticalStoreProducts: React.FC = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/api/optical/${id}/products?page=${currentPage}`,
          {
  ...
  const fetchPrescriptions = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/my-prescriptions/${patientProfileId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const submitOrder = async () => {
    if (!selectedPrescriptionId)
      return alert("Please select a prescription first");
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8]">
      <PatientNavbar />
      <h1 className="text-3xl font-bold text-[#1A2E44] text-center pt-28 mb-8">
        Store Inventory
      </h1>
          ...
          >
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <button
                ...
                className="bg-[#1A2E44] text-white px-4 py-2 rounded-lg hover:bg-black transition"
              >
                Add to Cart
              </button>
            </div>
          ...
          >
            ...
            <div className="p-6 border-t">
              ...
              <button
                ...
                className="w-full bg-[#1A2E44] text-white py-3 rounded-xl font-bold"
              >
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {selectedPrescriptionId === p.id && (
                          <CheckCircle className="text-[#1A2E44]" />
                        )}
                      </div>
                      <div className="grid grid-cols-2">
                        ...
                        <div className="block my-5">
                          <p className="font-bold">
                            Effective Period {p.effective_period}
                          </p>
                        </div>
                        <div className="block my-5">
                        ...
                        <div className="block my-5">
                          <p className="font-bold">
                            Left Sphere {p.left_sphere}
                          </p>
                        </div>
                        <div className="block my-5">
                          <p className="font-bold">
                            Right Sphere {p.right_sphere}
                          </p>
                        </div>
                        <div className="block my-5">
                          <p className="font-bold">
                            Left Cylinder {p.left_cylinder}
                          </p>
                        </div>
                        <div className="block my-5">
                          <p className="font-bold">
                            Right Cylinder {p.right_cylinder}
                          </p>
                        </div>
                      </div>
                    </div>
