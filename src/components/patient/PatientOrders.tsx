...
const PatientOrders: React.FC = () => {
  ...
  const fetchMyOrders = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/optical/orders/my-orders",
        {
  ...
  const handleReorder = async (orderId: number) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/optical/orders/${orderId}/reorder`,
        {
          method: "POST",
          headers: {
        ...
        <div className="flex justify-between items-end mb-10">
          <div>
            ...
            <p className="text-gray-500 mt-2">
              Track your eyewear orders and prescriptions
            </p>
          </div>
                ...
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        <h3 className="font-bold text-lg text-[#1A2E44]">
                          Order #{order.id}
                        </h3>
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${getStatusStyle(order.status)}`}
                        >
                          {order.status}
                        </span>
                      </div>
                      <div className="flex items-center gap-4 mt-1 text-sm text-gray-400">
                        <span className="flex items-center gap-1">
                          <Store size={14} /> {order.optical_store.store_name}
                        </span>
                        <span className="flex items-center gap-1">
                          <Clock size={14} />{" "}
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-[#1A2E44]">
                        ${order.total_price}
                      </p>
                    </div>
                    <button
                  ...
                  <div className="flex flex-wrap gap-3">
                    {order.items.map((item, idx) => (
                      ...
                      >
                        ...
                        <span className="text-xs bg-white px-2 py-0.5 rounded-md border text-gray-400">
                          x{item.quantity}
                        </span>
                      </div>
                    ))}
