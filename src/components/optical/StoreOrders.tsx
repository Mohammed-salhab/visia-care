...
const StoreOrders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/optical/orders/pending",
        {
  ...
  const handleAction = async (orderId: number, action: string) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/optical/orders/${orderId}/${action}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8]">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-28 pb-20">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-[#1A2E44]">
            Orders Management
          </h2>
          <p className="text-gray-500 mt-1">
            Manage pending orders and view prescription details.
          </p>
        </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-sm font-bold text-gray-600 text-center">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-400 font-bold">
                        Medication/Lens Type
                      </p>
                      <p className="font-medium text-gray-700">
                        {selectedPrescription.medication_name || "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 font-bold">
                      ...
                      <p className="font-medium text-gray-700">
                        {selectedPrescription.effective_period} Months
                      </p>
                    </div>
                  </div>
                </div>
