...
const StoreOrders: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/optical/${storeId}/products`,
        {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/optical/products/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Manage your store inventory and items.
            </p>
          </div>
          <button
            onClick={() => {
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-[#1A2E44] font-bold text-center">
                  Actions
                </th>
              </tr>
            </thead>
