...
const StoreOrders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/optical/orders/approved",
        {
  ...
  const markAsReady = async (orderId: number) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/optical/orders/${orderId}/ready`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <header className="mb-10">
          ...
          <p className="text-gray-500 mt-2">
            Manage customer orders and track prescriptions
          </p>
        </header>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#1A2E44] text-white">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-semibold text-sm uppercase text-center">
                      Action
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {order.items.map((item, idx) => (
                          <div
                            key={idx}
                            className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded mb-1 inline-block mr-1"
                          >
