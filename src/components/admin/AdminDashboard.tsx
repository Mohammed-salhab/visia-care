  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      try {
        ...
        const [pDocs, pOpts, aDocs, aOpts, rDocs, rOpts, notes] =
          await Promise.all([...adminRequests, notificationRequest]);

        setPendingDoctors(
          pDocs.data?.map((u: Item) => ({ ...u, type: "doctor" })) || [],
        );
        setPendingOpticals(
          pOpts.data?.map((u: Item) => ({ ...u, type: "optical" })) || [],
        );
        setApprovedDoctors(
          aDocs.data?.map((u: Item) => ({ ...u, type: "doctor" })) || [],
        );
        setApprovedOpticals(
          aOpts.data?.map((u: Item) => ({ ...u, type: "optical" })) || [],
        );
        setRejectedDoctors(
          rDocs.data?.map((u: Item) => ({ ...u, type: "doctor" })) || [],
        );
        setRejectedOpticals(
          rOpts.data?.map((u: Item) => ({ ...u, type: "optical" })) || [],
        );
        ...
        {activeSection === "dashboard" && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              ...
              <StatCard
                title="Pending"
                value={pendingDoctors.length + pendingOpticals.length}
                icon={<Clock />}
              />
              <StatCard
                title="Notifications"
...
const NotificationsSection = ({ notifications, loading }: any) => {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-3xl font-bold text-[#1A2E44] mb-6 tracking-tight">
        System Alerts
      </h2>
      {notifications.length === 0 ? (
        <div className="bg-white/50 p-20 rounded-3xl text-center text-gray-400 border-2 border-dashed border-gray-200">
        ...
        notifications.map((note: any) => (
          <motion.div
            ...
            key={note.id}
            className={`p-6 rounded-2xl bg-white shadow-sm border-l-4 ${note.read_at ? "border-gray-100 opacity-60" : "border-[#1A2E44]"}`}
          >
            <h3 className="font-bold text-[#1A2E44]">{note.title}</h3>
            <p className="text-gray-500 text-sm mt-1">{note.message}</p>
            <span className="text-[10px] text-gray-400 block mt-2 font-mono uppercase tracking-widest">
...
const StatCard = ({ title, value, icon }: any) => (
  <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
    <div className="text-[#1A2E44] bg-blue-50 w-10 h-10 flex items-center justify-center rounded-xl mb-4">
      {icon}
    </div>
...
const UsersTable = ({ users, approve, reject, loading, title }: any) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 bg-gray-50/50 border-b border-gray-100 font-bold text-[#1A2E44]">
        {title}
      </div>
                  ...
                  {u.status === "pending" ? (
                    <>
                      <button
                        ...
                        className="text-[10px] font-bold bg-[#1A2E44] text-white px-4 py-1.5 rounded-lg"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => reject(u.id, u.type)}
                        className="text-[10px] font-bold border border-red-100 text-red-500 px-4 py-1.5 rounded-lg"
                      >
                        Reject
                      </button>
                    </>
                    ...
                    <span
                      className={`text-[10px] font-black uppercase ${u.status === "approved" ? "text-green-500" : "text-red-300"}`}
                    >
                      {u.status}
                    </span>
                  )}
      ...
      <div className="flex gap-2">
        {["all", "pending", "approved", "rejected"].map((f) => (
          <button
            ...
            className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${filter === f ? "bg-[#1A2E44] text-white shadow-lg shadow-blue-900/20" : "bg-white text-gray-400 hover:text-gray-600 border border-gray-100"}`}
          >
            {f.toUpperCase()}
          </button>
        ))}
